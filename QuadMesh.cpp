...
void QuadMesh::DrawMesh(int meshSize)
{
	...
	for(int j=0; j< meshSize; j++)
	{
		for(int k=0; k< meshSize; k++)
		{
			...
			if (check == true) { //swaps between per vertice and per quad
			glNormal3f(quads[currentQuad].normal.x,
						quads[currentQuad].normal.y,
						quads[currentQuad].normal.z);
			}
...
void QuadMesh::ComputeNormals() 
{
	...
	for(int j=0; j< this->maxMeshSize; j++)
	{
		for(int k=0; k< this->maxMeshSize; k++)
		{
			...
			quads[currentQuad].normal.LoadZero();
			...
			quads[currentQuad].normal.Normalize();
...
//updates mesh and recomputes normals for each vertex/quad
void QuadMesh::UpdateMesh()
{
	int dx;
	int dz;

	for (int j = 0; j< numVertices; j++) //for each vertex, get the gaussian of each and add it to the vertex's height
	{
		float y = 0.0; 
		for (int k = 0; k<= blobCounter; k++)
		{
			float dx = pow(blob[k].x - vertices[j].position.x, 2); //subtraction of x from vertex to blob
			float dz = pow(blob[k].z - vertices[j].position.z, 2); //subtraction of z from vertex to blob
			y += blob[k].height * pow(2.71828182846, (-blob[k].width * (dx + dz))); //gaussian equation
		}
		vertices[j].position.y = y; //add the height calculated from gaussian to the current vertex
	}
	ComputeNormals(); //recompute normals
}

//creates a new blob
void QuadMesh::createBlob()
{
	blobCounter++; //make next blob
	blob[blobCounter].height = 1;
	blob[blobCounter].width = 1;
	blob[blobCounter].x = 0;
	blob[blobCounter].z = 0;
	UpdateMesh();
}

//moves the blob
void QuadMesh::moveBlob(float x, float z)
{
	blob[blobCounter].x += x;
	blob[blobCounter].z += z;
	UpdateMesh();
}

//scales the blob
void QuadMesh::change(float height, float width)
{
	if(blob[blobCounter].width + width > 0) //width only gets so wide
		blob[blobCounter].width += width;
	blob[blobCounter].height += height;
	UpdateMesh();
}

//toggles between quadNormal or vertex normal
void QuadMesh::quadNormal(bool checkR)
{
	check = checkR;
}

//move blob on mouse position
void QuadMesh::moveBlobMouse(float x, float z)
{
	blob[blobCounter].x = x;
	blob[blobCounter].z = z;
	UpdateMesh();
